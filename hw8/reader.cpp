...
void sigfunc(int sig) {
    ...
    if (sig == SIGINT) {
        ...
        printf("Reader(SIGINT) ---> Another Reader(SIGTERM)\n");
    } else if (sig == SIGTERM) {
        printf("Reader(SIGTERM) <--- Writer Or Reader(SIGINT)\n");
    }
...
int main() {
    ...
    buffer = (shared_memory *)mmap(0, sizeof(shared_memory), PROT_WRITE | PROT_READ, MAP_SHARED, buf_id, 0);
