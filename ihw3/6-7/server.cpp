...
int main(int argc, char *argv[]) {
    ...
    for(;;) {
        ...
        if ((clientsock3 = accept(serversock, (struct sockaddr *) &echoClient, &recvMsgSize)) < 0) {
            DieWithError("Failed to accept client connection");
        } else {
            fprintf(stdout, "Client3 connected: %s\n", inet_ntoa(echoClient.sin_addr));
        }
        ...
        while (totalBytesRcvd > 0) {
            messegesReceived++;
            if (send(clientsock2, buffer, totalBytesRcvd, 0) != totalBytesRcvd || send(clientsock3, buffer, totalBytesRcvd, 0) != totalBytesRcvd) {
